...
describe('Protected API', () => {
  ...
  describe('Response modification', () => {
    ...
    it('should send request with signals as application/json to ingress and return modified response', async () => {
      const { getIngressRequest, getOriginRequest } = prepareMockFetch({
        mockIngressHandler: async () => {
          const headers = new Headers()
          headers.append('Set-Cookie', 'fp-ingress-cookie=12345')
      ...
      const originBody = (await getOriginRequest()!.json()) as Record<string, unknown>
      expect(originBody[SIGNALS_KEY]).toBeUndefined()
    })
