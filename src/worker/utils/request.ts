...
export function setCorsHeadersForInstrumentation(request: Request, originResponseHeaders: Headers) {
  ...
  if (allowedOrigin && allowedOrigin !== 'null') {
    // A cross-origin request is being allowed by the origin server or for a
    // block response generated by this worker. Add the additional CORS headers
    ...
    const exposeHeaders = originResponseHeaders.get('Access-Control-Expose-Headers')
    if (exposeHeaders) {
      originResponseHeaders.set('Access-Control-Expose-Headers', `${exposeHeaders}, ${AGENT_DATA_HEADER}`)
    } else {
      originResponseHeaders.set('Access-Control-Expose-Headers', AGENT_DATA_HEADER)
    }
  }
}
