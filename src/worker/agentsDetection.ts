...
export async function storeToken(request: Request, env: TypedEnv): Promise<Response> {
  ...
  const validationResponse = await validateRequestId(request, reportRequest.requestId, env)
  if (validationResponse) {
    return validationResponse
  }

  const tokenStore = env.TokenStore.getByName(GLOBAL_TOKEN_STORE)
  await tokenStore.storeRequestId(reportRequest.token, reportRequest.requestId)
...
async function updateTagsForEvent(
  ...
  await fpClient
    ...
    .catch(async (error) => {
      ...
      if (error instanceof RequestError && error.statusCode === 490) {
...
export async function handleDetectionTokenRequest(url: URL, request: Request, env: TypedEnv): Promise<Response> {
  try {
    ...
    if (token) {
      const tokenStore = env.TokenStore.getByName(GLOBAL_TOKEN_STORE)
      const requestId = await tokenStore.getRequestId(token)

      if (!requestId) {
        console.warn(`Received request with unknown token: "${token}"`)
        console.debug('all tokens:', await tokenStore.listAll())
      } else {
